...
**EVV5 全库复量（GROW 2.1-B R83，2026-07-15）**：place 页 165→**199**，超 400 字长段仍 **24 页**
（净增建 34 页 R76–R82 全 ≤400，无一新增违规）。max 上修为 bay-of-talcahuano **614**、rocky-mountains 596、
strait-of-gibraltar 568、liedenbrock-sea 541（较 R72 的 max=498 更高，因当时统计漏含这些既有页的最长段；
均 R60 前既有页，非 R64–R82 新建）。**模板仍稳定**（199 页 4-H2 齐备、7 必填字段齐备、real_or_fictional
合法）。处置同上 DEFERRED，不改。

### HK-R78-tsalal-dup — 建页未交叉核既有页 → 同实体重复页（LAW §10 label 冲突）

**现象（GROW 2.1-B R81，2026-07-15 发现并处置）**：R78 NEW1 建 `tsalal`（An Antarctic Mystery
Poe Pym 南极岛），与既有 `tsalal-island`（R37 建，正页，halbrane-land 反链）为**同一实体重复页**，
两页共享 label `Tsalal Island` → `build_registry.py` 报 `alias conflict`。根因：**建页前交叉核
pages.json 既有页/label 的纪律 R79 起才落地**，R77/R78 建页在此纪律之前，未做既有页比对。

**处置**：R81 `git rm docs/wiki/pages/ts/tsalal.md` 撤销我方重复页，正页 tsalal-island 承载
`[[Tsalal Island]]`，反链无损（halbrane-land 原即指向 tsalal-island）。registry 冲突清零（仅剩
Robur 旧账 PARK）。**已修复**，非遗留债；本条仅存证根因，供后续审计参考。

**后续建议**：R84 discover 轮附带补核 R77 建的 5 页（nijni-novgorod/bombay/melbourne/lima/gibraltar）
是否另有既有重复页。R82 起五港建前均已交叉核 pages.json，无既有页。
...
### HK-corpus-discover-not-in-statemachine — priority 3.5 corpus-discover 未同步进权威决策算法

**现象（GROW 2.1-B R106 决策时发现，2026-07-16）**：`GROW.spec.md` 的决策矩阵**汇总表**列有
priority 3.5「`rounds_since_last_corpus_discover >= corpus_discover_interval` 且 `corpus_exhausted==false`
→ `SCN28-corpus`」（RFC-memex-0071 引入），但该表明文注「完整算法见 `grow-state-machine.md §3`」，
而 **`grow-state-machine.md §3` 的权威决策伪码根本不含 corpus-discover 分支**——其序列为
EVV5 → CLOSE → SCN28(queue<threshold) → 僵尸 guard(queue(type)==0) → RCH2 → NEW1+RCH2 → NEW1。

**影响**：两文档不一致。`rounds_since_last_corpus_discover` 自 GROW R94 起连续 ≥30（本 wiki 现=43）
却从未触发 corpus-discover——因操作性算法（状态机伪码）无此分支，NEW1/SCN28 一直正常接管。
即：**按权威算法，since_corpus 计数器实际是死变量**，priority 3.5 从未生效。历轮日志（含本对话）
将其记为「名义触发但顺延」属保守表述，结论（走 NEW1/SCN28）与权威算法一致，无误建。

**修复方向**：二选一——(a) 将 priority 3.5 分支补入 `grow-state-machine.md §3` 伪码（在 SCN28 与
RCH2 之间），使 corpus-discover 真正生效；或 (b) 若有意弃用 corpus-discover，从 `GROW.spec.md`
汇总表删除 priority 3.5 及 corpus_* 计数器/阈值，消除死变量。

**处置**：属共享 memex 规范（`GROW.spec.md` + `grow-state-machine.md`）—— 依 RFC 停靠决策 **PARK**，
不自动起 RFC（须走 memex RFC 流程）；与 featured-inflation / registry-typefield / VVV 宽度诸项一并
用户末期批量评审。本地无需改动，决策继续依权威算法（状态机伪码）执行。
