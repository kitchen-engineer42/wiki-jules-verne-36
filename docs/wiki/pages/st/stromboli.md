---
id: stromboli
type: place
label: Stromboli
aliases: []
tags: [place, volcano, exploration]
description: 'The Italian volcano through whose crater the explorers are expelled at the end of Journey to the Center of the Earth.'
book: Journey to the Center of the Earth
real_or_fictional: real
region: Aeolian Islands, Italy
quality: featured
---
## Overview

Stromboli is the real Italian volcano that serves as the astonishing exit of *[[journey-to-the-center-of-the-earth|Journey to the Center of the Earth]]*. Having descended into the Earth in [[iceland|Iceland]], the explorers are hurled back to the surface through a wholly different volcano, and their disbelief is captured in a single cry: "Stromboli! What an effect this unexpected" name produces (JCE-044-053), repeated in wonder — "Stromboli, Stromboli!" (JCE-044-054).

The volcano thus forms the southern counterpart to the [[iceland|Iceland]]ic entrance, closing the subterranean journey by ejecting the travelers into the Mediterranean sun.

## In the Narrative

Stromboli is the site of the expedition's violent emergence. Carried upward on a rising column of lava, the party comes "out of the crater half naked" (JCE-044-010), saved from the fire at the last moment. Above them they see "the crater of a volcano" through which they have been expelled (JCE-044-016), the vent that has just vomited them onto the mountainside.

The eruption is still ongoing as they escape. The narrator notes wryly that it "would not look well to have come out by an erupt[ion]" (JCE-044-022) — yet that is exactly how the journey ends. Descending the slopes, they reach the port of Stromboli (JCE-044-057), where "the Stromboliotes received us kindly as shipwrecked strangers" (JCE-045-003).

## Geography & Features

Stromboli is an active volcanic island in the Mediterranean, and the novel emphasizes its live crater. The explorers look up at the vent hundreds of feet above them, "the crater of a volcano" in continuous eruption (JCE-044-016), streams of lava running down its declivities toward the sea.

As a real, inhabited island it has a settled community — the Stromboliotes — who take the strangers in (JCE-045-003). This inhabited, sunlit Mediterranean island stands in deliberate contrast to the bleak [[iceland|Iceland]]ic volcano where the descent began.

## Connections

Stromboli is the exit of the underground voyage that began at [[Snæfellsjökull]] in [[Iceland]] and passed across the [[Liedenbrock Sea]], the volcano through which [[Professor Lidenbrock]] and his companions return to the surface in *[[Journey to the Center of the Earth]]*.
